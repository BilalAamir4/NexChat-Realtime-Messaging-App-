...
class _GroupInfoScreenState extends State<GroupInfoScreen> {
  ...
  Future<void> _loadGroupInfo() async {
    try {
      ...
      if (mounted) {
        setState(() { _groupName = groupName; _members = members; _loading = false; });
      }
    } catch (_) {
  ...
  Future<void> _leaveGroup() async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        title: Text('Leave Group',
            style: TextStyle(color: context.textPrimary, fontWeight: FontWeight.w800, fontSize: 17)),
    ...
    try {
      ...
      if (mounted) {
        Navigator.pushNamedAndRemoveUntil(context, AppRoutes.groups,
      ...
      builder: (context) => AlertDialog(
        ...
        content: TextField(
          ...
          decoration: InputDecoration(
            ...
            filled: true,
            fillColor: context.receivedBubbleBg,
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context)
            .showSnackBar(const SnackBar(content: Text('Failed to update group name.')));
      }
    }
  }
  ...
  Widget _buildAvatar({required double radius, String? photoUrl, String? name, bool isGroup = false}) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: NexColors.indigo.withValues(alpha: 0.25), blurRadius: 16, offset: const Offset(0, 4))],
      ),
      child: photoUrl != null && photoUrl.isNotEmpty
        ...
        child: isGroup
            ...
            style: TextStyle(color: Colors.white, fontSize: radius * 0.75, fontWeight: FontWeight.w800))
            : Icon(Icons.group_rounded, color: Colors.white, size: radius * 0.8))
            : (name != null && name.isNotEmpty
            ? Text(name[0].toUpperCase(),
  ...
  Widget _buildMemberTile(Map<String, String> member) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: NexColors.indigo.withValues(alpha: 0.06), blurRadius: 10, offset: const Offset(0, 4))],
      ),
      child: Row(
        children: [
          _buildAvatar(radius: 22, photoUrl: member['photo'], name: member['name']),
          ...
          if (isMe)
            Container(
              ...
              child: const Text('You',
                  style: TextStyle(color: NexColors.indigo, fontSize: 11, fontWeight: FontWeight.w700)),
            ),
        ],
      ),
    );
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Text('Group Info',
                          style: TextStyle(color: context.textPrimary, fontWeight: FontWeight.w800, fontSize: 20, letterSpacing: -0.4)),
                    ),
                  ],
                ),
              ),
                ...
                child: ListView(
                  ...
                  children: [
                    Center(
                      child: Column(
                        children: [
                          _buildAvatar(radius: 46, name: _groupName, isGroup: true),
                          const SizedBox(height: 16),
                          Row(
                            mainAxisSize: MainAxisSize.min,
                            children: [
                              Text(_groupName,
                                  style: TextStyle(color: context.textPrimary, fontWeight: FontWeight.w800, fontSize: 22, letterSpacing: -0.4)),
                              ...
                              GestureDetector(
                                ...
                                child: Container(
                                  ...
                                  child: const Icon(Icons.edit_rounded, color: NexColors.indigo, size: 15),
                                ),
                              ),
                            ],
                          ),
                          ...
                          Container(
                            ...
                            child: Text('${_members.length} member${_members.length == 1 ? '' : 's'}',
                      ...
                      child: Row(
                        children: [
                          Container(
                            ...
                            child: const Icon(Icons.people_rounded, color: NexColors.indigo, size: 16),
                          ),
                          const SizedBox(width: 8),
                          Text('Members',
                    ...
                    GestureDetector(
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          color: context.isDark ? const Color(0xFF2A1515) : const Color(0xFFFEF2F2),
                          borderRadius: BorderRadius.circular(16),
                          border: Border.all(color: const Color(0xFFFCA5A5), width: 1),
                          boxShadow: [BoxShadow(color: const Color(0xFFEF4444).withValues(alpha: 0.1), blurRadius: 12, offset: const Offset(0, 4))],
                        ),
                        child: Center(
                          child: _leaving
                            ...
                            mainAxisSize: MainAxisSize.min,
                            children: [
                              Icon(Icons.exit_to_app_rounded, color: Color(0xFFEF4444), size: 20),
                              SizedBox(width: 8),
                              Text('Leave Group',
